...
<template>
  <div class="min-h-screen flex flex-col font-sans overflow-x-hidden" @keydown="onKeydown">
      ...
      <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav class="flex items-center gap-4 text-sm font-medium">
          <NuxtLink to="/problems"
            class="hidden sm:inline-flex items-center min-h-[44px] px-2 text-white hover:opacity-80 transition-opacity font-medium">
            Problems
          </NuxtLink>
          <NuxtLink to="https://github.com/OpenSauce/dsa-flash-cards" target="_blank"
            class="hidden sm:inline-flex items-center justify-center min-h-[44px] min-w-[44px] text-white hover:opacity-80 transition-opacity">
          ...
          <div>
            <div v-if="authReady">
              <div v-if="!isLoggedIn" class="flex items-center gap-3">
                <NuxtLink to="/login"
                  class="inline-flex items-center min-h-[44px] px-3 rounded-md transition-transform hover:scale-105 hover:bg-white/10">Log&nbsp;in
                </NuxtLink>
                <NuxtLink to="/signup"
                  class="inline-flex items-center bg-white text-purple-600 font-semibold min-h-[44px] px-4 rounded-md transition-transform hover:scale-105 hover:bg-white/90">
                  Sign&nbsp;up
                </NuxtLink>
              </div>
              <div v-else>
    ...
    <footer class="border-t text-center py-4 text-sm text-gray-600">
      ...
      <span class="mx-2 text-gray-300 inline" aria-hidden="true">·</span>
      <NuxtLink to="/about" class="hover:underline">About DSA Flash</NuxtLink>
    </footer>
  </div>
</template>
